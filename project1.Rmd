...
pvalColumns <- c("Control.Acarbose_PValue", "Control.CR_PValue", "GHRKOControl.GHRKO_PValue", 
                 "Control.Rapamycin_PValue", "SnellDwarfMiceControl.SnellDwarfMice_PValue", 
                 "Control.Protandim_PValue", "Control.alphaestradiol_PValue", 
                 "MRControl.MethionineRestriction_PValue")
...
Plot DEG results
```{r deg results}
edgeDF <- read.delim("./mouseedgeRcomp.xls", row.names = 1, check.names = FALSE)
DEG_list <- filterDEGs(degDF = edgeDF, filter = c(Fold = 2, FDR = 20))
```

Venn diagram for Control-Acarbose, Control-CR, Control-Rapamycin, Control-Protandim, Control-Protandim
```{r venn diagram}
